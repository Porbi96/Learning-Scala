object Lab3 {
  ...
  def minmax(vals: List[Int]): Tuple2[Int, Int] = {
    @scala.annotation.tailrec
    def _minmax(vals: List[Int], min: Int, max: Int): Tuple2[Int, Int] = {
      if (vals.nonEmpty) {
        val _min = if (vals.head < min) vals.head else min
        val _max = if (vals.head > max) vals.head else max
        _minmax(vals.tail, _min, _max)
      } else (min, max)
    }

    _minmax(vals, vals.head, vals.head)
  }
...
//
//    val (min, max) = minmax(List(3,2,3,4,5,4))
//
//    // ======================================================================
////    println("pi:" + pi)
////    println("pi(pi): " + pi(pi))
